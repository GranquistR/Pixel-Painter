...
const localColor = computed({
  ...
  set: (newColor) => emit("update:modelValue", newColor), // Send update to parent
});
...
function drawSquare() {
  ...
  ctx.value.fillStyle = localColor.value; // Apply color
  ctx.value.fillRect(0, 0, 100, 100); // Draw square
}
